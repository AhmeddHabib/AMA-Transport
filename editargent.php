...
{	
	...
	if(empty($tel_emetteur) || empty($tel_recepteur) ) {	
			
		if(empty($tel_emetteur)) {
			echo "<font color='red'>Attention il faut entrer la numero de telephone de l'emetteur .</font><br/>";
		}
		
		if(empty($tel_recepteur)) {
			echo "<font color='red'>Attention il faut entrer la numero de telephone du recepteur .</font><br/>";
		}
		
				
	} else {	
		//updating the table
		$result = mysql_query("UPDATE trans_argents SET id_argent='$id_argent', tel_emetteur='$tel_emetteur',tel_recepteur='$tel_recepteur',montant='$montant',destination='$destination' ,date='$date',num_navette='$num_navette' WHERE id_argent=$id_argent");
...
$result = mysql_query("SELECT * FROM trans_argents WHERE id_argent=$id_argent");
 
while($res = mysql_fetch_array($result))
{
	
	
	$tel_emetteur = $res['tel_emetteur'];
	$tel_recepteur = $res['tel_recepteur'];
	$montant = $res['montant'];
...
<br><br>
  ...
  <h2>Modifier ce Passager</h2>
	<form name="form1" method="post" action="editargent.php">
		<table border="0" bgcolor="">
			
			<tr> 
				<td>Tel Emetteur</td>
				<td><input type="text" name="tel_emetteur" value=<?php echo $tel_emetteur;?>></td>
			</tr>
			<tr> 
				<td>Tel Recepteur</td>
				<td><input type="text" name="tel_recepteur" value=<?php echo $tel_recepteur;?>></td>
			</tr>
			<tr> 
				<td>Montant</td>
 ...
 name="date" value=<?php echo $date;?>></td>
			...
			<tr>
				<td><input type="hidden" name="id_argent" value=<?php echo $_GET['id_argent'];?>></td>
